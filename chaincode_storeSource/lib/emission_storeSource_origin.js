...
//CreateSource
    ...
    async CheckRegistry(ctx,CompanyId) {
        ...
        const origin_result= await ctx.stub.invokeChaincode('emission_registry19',['CheckRegistry',CompanyId,Address],'carbontest');
    ...
    async CheckAuthority(ctx,CompanyId) {
        const Address= await this.GetMsgSender(ctx);
        const origin_result= await ctx.stub.invokeChaincode('emission_registry19',['CheckAuthority',CompanyId,Address],'carbontest');
        const result = origin_result.payload.toString();
        return result;
    }
}
